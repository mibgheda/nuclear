			...
			if (formula != "") {
				...
				if (neutronFormula != null) {
					if (exceptionP == 'revert') {
						//спец костыль чтобы не тратить много времени, передадим этот несчастный реверт в атрибуте exceptionP
						//буква S
						revert = true
				...
				if (sflour_p == 1 && sindex_p == 1) {
					...
					(neutronFormula[0].toString() == "null" || revert) ? oneos = true :	oneos = false;
